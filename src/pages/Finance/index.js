    ...
    async function fetchData() {
      try {
        if (dataType === 'Saldo Total') {
          ...
          const response = await Api.post('/SalesRecord/getAll', {
            userId: UserId,
          });
          const RawData = response.data;
  ...
  return (
    <View style={styles.container}>
      ...
      <Modal isVisible={modalVisible}>
        <View
          ...
          <LightInput
            Control={control}
            Name={'FixedCosts'}
            Placeholder={'Custos Totais por mês'}
      ...
      <View
        ...
        <InputArea
          onPress={() => {
            setDataType('Saldo Total');
          }}>
          <NewText>Ver saldo total</NewText>
        </InputArea>
        <InputArea
          onPress={() => {
            setDataType('Vendas');
          }}>
          <NewText>Ver Vendas</NewText>
        </InputArea>
        <InputArea
          onPress={() => {
            setModalVisible(true);
          }}>
          <NewText>Simular Lucro ao Fim do mês</NewText>
        </InputArea>
      </View>
    </View>
  );
